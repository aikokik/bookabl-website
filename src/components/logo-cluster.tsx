...
function MainLogo() {
  return (
    <div className="absolute left-44 top-32 flex size-16 items-center justify-center rounded-full bg-white shadow ring-1 ring-black/5">
      <Logo src="/logo.png" left={0} top={0} hover={{ x: 0, y: 0, rotate: 0, delay: 0 }} />
    </div>
  )
}
  ...
  return (
    <div aria-hidden="true" className="relative h-full overflow-hidden">
      ...
      <div className="absolute left-1/2 h-full w-[26rem] -translate-x-1/2">
        <MainLogo />
        <Logo
          src="/logo-cluster/dmn.png"
          left={360}
          top={144}
          hover={{ x: 6, y: 1, rotate: 5, delay: 0.38 }}
        />
        <Logo
          src="/logo-cluster/opentable.png"
          left={285}
          top={20}
          hover={{ x: 4, y: -5, rotate: 6, delay: 0.3 }}
        />
        <Logo
          src="/logo-cluster/quandoo.png"
          left={255}
          top={210}
          hover={{ x: 3, y: 5, rotate: 7, delay: 0.2 }}
        />
        <Logo
          src="/logo-cluster/resy.jpg"
          left={144}
          top={40}
          hover={{ x: -2, y: -5, rotate: -6, delay: 0.15 }}
        />
        <Logo
          src="/logo-cluster/sevenrooms.jpeg"
          left={36}
          top={56}
          hover={{ x: -4, y: -5, rotate: -6, delay: 0.35 }}
        />
        <Logo
          src="/logo-cluster/tock.png"
          left={96}
          top={176}
          hover={{ x: -3, y: 5, rotate: 3, delay: 0.15 }}
        />
        <Logo
          src="/logo-cluster/thefork.png"
          left={150}
          top={255}
          hover={{ x: -2, y: 4, rotate: 3, delay: 0.15 }}
        />
      </div>
    </div>
  )
